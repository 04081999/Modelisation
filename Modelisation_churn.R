...
#library(gtsummary) # Générer des tableaux permettant de resumer l'ens des Stats Descrip et faire des tests stats
...
state      account_length  area_code  international_plan
  plus_riches    :2105   Min.   :  1.0   408: 838   Non:3010          
 moyennent_riches: 607   1st Qu.: 74.0   415:1655   Oui: 323          
 assez_riches    : 621   Median :101.0   510: 840                     
                         Mean   :101.1                                
                         3rd Qu.:127.0                                
                         Max.   :243.0                                
 voice_mail_plan number_vmail_messages total_day_minutes total_day_calls
 Non:2411        Min.   : 0.000        Min.   :  0.0     Min.   :  0.0  
 Oui: 922        1st Qu.: 0.000        1st Qu.:143.7     1st Qu.: 87.0  
 ...
 Min.   : 0.000   Min.   :0.000     Min.   :0.000          Non:2850  
 1st Qu.: 3.000   1st Qu.:2.300     1st Qu.:1.000          Oui: 483  
...
dim(train)
prop.table(table(train$churn))# Même répartition (churn = 85 et 15%)

# Set test
test = data_telecom_dummy[-index,]
dim(test) 
prop.table(table(test$churn))# Même répartition (churn =85 et 15%)
#-------------------------------------------------------------------------------------------
...
# Downsample
set.seed(99)
train_downSample = downSample(x = train |> select(-churn),
                              y = as.factor(train |> pull(churn)),
                              yname = "churn")
# Repartition des Classes après le downsample
table(train$churn, deparse.level = 2) # Au depart
table(train_downSample$churn,deparse.level = 2)


# ------------> UpSample
set.seed(99)
train_upSample = upSample(x = train |> select(-churn),
                          y = as.factor(train |> pull(churn)),
                          yname = "churn")
# Repartition des classes après le upsample
table(train$churn, deparse.level = 2) # Au depart
table(train_upSample$churn,deparse.level = 2)

#####------------------------------------------------------------------------------------------------------------------------------


# 5.2  Modèle de Machine Learning -----------------------------------------------------------------
"
...
tic()
set.seed(99)
xgbTree = train(churn~. ,data = train_downSample ,
                method = "xgbTree",
...
Summary of sample sizes: 697, 697, 696, 696, 697, 696, ... 
Resampling results across tuning parameters:

  eta   max_depth  ROC        Sens       Spec     
  0.01   2         0.8795573  0.8544309  0.8052182
  0.01   3         0.8872818  0.8888214  0.8138776
  0.01   5         0.9067444  0.9129780  0.8294422
  0.01  10         0.8981309  0.8897661  0.8311741
  0.10   2         0.9004494  0.8690733  0.8404408
  0.10   3         0.9048954  0.8819163  0.8430724
  0.10   5         0.9082905  0.9026991  0.8500675
  0.10  10         0.9068506  0.9069951  0.8475034
  0.30   2         0.8987313  0.8759784  0.8456140
  0.30   3         0.8978417  0.8795322  0.8422627
  0.30   5         0.9057557  0.8966262  0.8509222
  0.30  10         0.9098260  0.8992128  0.8552182
...
tic()
set.seed(99)
svm = train(churn~. ,data = train_downSample , 
            method = 'svmRadial',
            preProcess = NULL,
            trControl = Control_roc  , 
            tuneLength = 8,
...
"
   C         ROC        Sens       Spec     
   0.25  0.8636218  0.8088619  0.7959289
   0.50  0.8732790  0.8286100  0.8165542
   1.00  0.8779374  0.8208727  0.8224696
   2.00  0.8813731  0.8286550  0.8268106
   4.00  0.8776995  0.8226271  0.8078722
   8.00  0.8728845  0.8295996  0.7932299
  16.00  0.8666715  0.8243140  0.7769231
  32.00  0.8520774  0.8106838  0.7718623

Tuning parameter 'sigma' was held constant at a value of 0.02488572
ROC was used to select the optimal model using the largest value.
The final values used for the model were sigma = 0.02488572 and C = 2.

"
# Comparaison des deux modèles à l'aide de la courbe ROC
...
# EVALUATION DES CLASSES PREDITES 
table(predicted)
...
conf_matrix <- confusionMatrix(
  data = predicted,                     # les prédictions
  reference = as.factor(pull(test, churn)), # les vraies valeurs
  positive = "Oui",                     # Classe positive
  mode = "everything"                   # pour afficher toutes les métriques
)
...
truth_predicted = data.frame(
  truth = test |> pull(churn),
  predicted = predicted)
# Créer la matrice objet 'cm'
cm = conf_mat(data = truth_predicted,
              truth,
              predicted)

# VISUALISER LA MATRICE DE CONFUSION
autoplot(cm,type = "heatmap")+
  ggtitle("Matrice de confusion du model xgbTree ")
...
" INTERPRETATION  : 
A travers le graphe ci_desssus, (importance variable), on voit que le Total des minutes  d'appels de la journée 
(total day minutes), la non souscription aux forfaits d'appels internationaux(International_Plan_Non) , 
les appels fréquents au service client(costumer_service_calls) sont les principaux facteurs ayant un fort
impact sur la prediction du desabonnement(churn) chez les clients .
